...
class testPPPlogging extends StompPFPPendingProcessorSA {
	...
	public function pubFetch_message($queue) {
		return $this->fetch_message($queue);
	}

	public function pubQueue_message($queue, $message) {
		return $this->queue_message($queue, $message);
	}
...
//		echo "THIS IS ABOUT TO DIE.\n";
//		echo "Looking for : " . print_r($testmessage, true) . "\n";
//		global $logdata;
//		echo print_r($logdata, true);
//		echo "Find it in there. \n";
	...
	public function testAckLogs() {
		//this should insert a message, and then retrieve them and ack.
		//Attempting to ack message...
		//Result of message ack: $sent"
	}
	...
	public function testExecuteLogs() {
		...
		$this->assertEmptyQueueWithAck($this->processor->activemq_pending_queue);
		$added_ids = $this->assertAddTestMessagesToQueue(10, $this->processor->activemq_pending_queue, "exe_");

		$this->processor->setTestMode();
	...
	public function assertAddTestMessagesToQueue($count, $queue, $tracker = "") {
		...
		for ($i = 0; $i < $count; ++$i) {
			...
			$this->processor->pubQueue_message($queue, json_encode($tempmessage));
			$this->assertTrue($this->checkLog("Attempting to queue message..."), "Message Queue Attempt $i was not logged");
			$this->assertTrue($this->checkLog("Result of queuing message:", true), "Message Queue Result $i was not logged");
		}
		return $ret;
	}
	...
	public function assertEmptyQueueWithAck($queue) {
		...
		$message = $this->processor->pubFetch_message($queue);
		$i = 1;
		$messages = array();
		while (!empty($message)) {
			$messages[] = $message;
		...
		foreach ($messages as $key => $message) {
			$this->processor->ackMessage($message);
			//and check the logs
